...
void IMU_85::i2cInit() {
    ...
    if (digitalRead(_sda_pin) == LOW) {
        ...
        for (int i = 0; i < 9; ++i) {
            digitalWrite(_scl_pin, LOW);  delayMicroseconds(5);
            digitalWrite(_scl_pin, HIGH); delayMicroseconds(5);
        }
        // STOP condition
        pinMode(_sda_pin, OUTPUT);
        digitalWrite(_sda_pin, LOW);  delayMicroseconds(5);
        digitalWrite(_scl_pin, HIGH); delayMicroseconds(5);
        digitalWrite(_sda_pin, HIGH); delayMicroseconds(5);
    }
}