...
public:
    ...
    // Raw quaternion from the rotation vector report.
    // qr = real/w, qi = i/x, qj = j/y, qk = k/z
    // Returns true if a fresh sample was available.
    bool getQuaternion(float& qr, float& qi, float& qj, float& qk, float& accuracy);

    // Convenience wrapper — converts quaternion to Euler (deg).
    // Still available for diagnostics / serial logging.
...
private:
    ...
    void i2cInit();
    void quatToYPR(float qr, float qi, float qj, float qk,
